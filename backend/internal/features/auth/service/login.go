...
func (s *AuthService) LoginUser(ctx context.Context, email, password string) (*string, *domain.User, error) {
	...
	if err != nil {
		...
		if err != nil {
			if errors.Is(err, pgx.ErrNoRows) {
				return nil, nil, fmt.Errorf("no user: %w", domain.UNAUTHORIZED)
			}

			return nil, nil, fmt.Errorf("get from database: %w", domain.INTERNAL_ERROR)
		}
	...
	if !hash_utils.CheckPasswordHash(password, user.Password) {
		return nil, nil, fmt.Errorf("wrong password: %w", domain.UNAUTHORIZED)
	}

	jwt, err := jwt_utils.GenerateJWT(user.ID, user.Role)
	if err != nil {
		return nil, nil, fmt.Errorf("generating jwt: %w", domain.INTERNAL_ERROR)
	}
